...
# Option 3: Get pose estimation (requires camera calibration)
def detect_with_pose(camera_matrix, dist_coeffs, tag_size):
    """
    ...
    while True:
        ...
        cv2.imshow('AprilTag with Pose', frame)
...
if __name__ == "__main__":
    ...
    # For pose estimation (requires calibrated camera):
    # camera_matrix = np.array([[800, 0, 320], [0, 800, 240], [0, 0, 1]], dtype=float)
    # dist_coeffs = np.zeros((5,1))
    # tag_size = 0.05  # 5cm tags
    # detect_with_pose(camera_matrix, dist_coeffs, tag_size)